...
public class FluidShooterMenu extends AbstractContainerMenu {
    ...
    public FluidShooterMenu(int containerId, Inventory inv, ContainerData data, @Nullable ItemStackHandler stackHandler){
        ...
        inputSlot = new SlotItemHandler(inventory, INPUT_SLOT, 47, 37);
        outputSlot = new SlotItemHandler(inventory, OUTPUT_SLOT, 115, 37);

        FluidShooterState state = ItemGunFluid.getState(inv.player.getItemInHand(InteractionHand.MAIN_HAND));

        ItemStack input = state.input();
        ItemStack output = state.output();
        int fuelAmount = state.amount();
        Fluid fuelFluid = BuiltInRegistries.FLUID.get(ResourceLocation.bySeparator(state.fluidId().toString(), ':'));
